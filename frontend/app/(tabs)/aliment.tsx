...
import AlimentMenu from "@/components/NavBar/AlimentMenu";
...
export default function AlimentScreen() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const objectifCO2 = userResult.user.objectif_co2_mensuel_aliment_creation;
            ...
            const eventsResult = await printAllEventsByCategorie('aliment', periode, "Tout");
            const events = eventsResult.status ? eventsResult.events : [];
            ...
            const eventsListResult = await printAllEventsByCategorie('aliment', periode, mapFiltreForService(filtre), objectifCO2);
            const iterationsFiltered = eventsListResult.status ? eventsListResult.events : [];
            setIterationsList(iterationsFiltered);

            const allEventsResult = await printAllEventsByCategorie('aliment', 'Cette année', "Tout");
            const moyenneCO2 = await getMoyenneCo2OfPreviousEvents(allEventsResult.events, periode);
            setMoyennePrecedente(moyenneCO2.moyenne_co2?.creation || 0);
        ...
        <View style={{ flex: 1 }}>
            <ScrollView keyboardShouldPersistTaps="handled" style={{ padding: 16, marginHorizontal: 10 }}>
                <Text style={{ fontSize: 24, fontWeight: "bold", marginBottom: 20 }}>
                    Mes émissions liées à l’Alimentation
                </Text>
            ...
            <AlimentMenu onCreated={fetchData} />

            {isTutoActive && (() => {
                const step = tutoSteps.aliment[currentStep];
                if (!step) return null;
