...
export default function RepetTransport({ onClose, referenceId, actionId }: Props) {
  const [date, setDate] = useState<Date>(new Date());
  const [endDate, setEndDate] = useState<Date | null>(null); // date de fin facultative
  ...
  const [repeatUnit, setRepeatUnit] = useState<'jours' | 'semaines' | 'mois' | 'années'>('jours');
    ...
    <View style={styles.overlay} pointerEvents="box-none">
      <View style={styles.modal}>
        <ScrollView contentContainerStyle={{ paddingBottom: 10 }} keyboardShouldPersistTaps="handled">
          <Text style={styles.title}>À quelle date voulez-vous refaire cette action ?</Text>
